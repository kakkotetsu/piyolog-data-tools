...
離乳食パネルは選択期間内の記録を「日時」「メモ」の2列で表示します。日時はブラウザのタイムゾーンで表示し、メモなしの記録も空欄で表示します。体温は時系列グラフ、その他のイベントは下部の「イベント」パネルで確認できます。

「睡眠」はState timelineで、日本時間の20:00〜08:00の睡眠を青、08:00〜20:00の昼寝を赤、覚醒・不明を透明で表示します。08:00・20:00をまたぐ睡眠は途中で色が変わります。現在時刻より先は不明扱いです。色分けは日本時間固定です。
